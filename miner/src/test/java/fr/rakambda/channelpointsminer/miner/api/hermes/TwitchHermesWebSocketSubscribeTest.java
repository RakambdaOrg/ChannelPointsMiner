...
class TwitchHermesWebSocketSubscribeTest{
	...
	void onResponse(WebsocketMockServer server) throws InterruptedException{
		...
		server.send(TestUtils.getAllResourceContent("api/hermes/subscribe_ok.json"));
		
		var expected = SubscribeResponse.builder()
				...
				.subscribeResponse(SubscribeResponse.SubscribeResponseData.builder()
						.result("ok")
						.build())
				.build();
