...
class TwitchHermesWebSocketAuthenticateTest {
	...
	void onResponse(WebsocketMockServer server) throws InterruptedException{
		...
		server.send(TestUtils.getAllResourceContent("api/hermes/authenticate_ok.json"));
		
		var expected = AuthenticateResponse.builder()
