...
public class PostgreSqlDatabase extends BaseDatabase {
    ...
    @Override
    protected void addUserPrediction(@NonNull String channelId, int userId, @NonNull String badge)
        throws SQLException {
    ...
    @Override
    protected void resolveUserPredictions(double returnRatioForWin, @NonNull String channelId,
        @NonNull String badge) throws SQLException {
        try (var conn = getConnection(); var getOpenPredictionStmt = conn.prepareStatement("""
    ...
    public void deleteUserPredictionsForChannel(@NonNull String channelId) throws SQLException {
        ...
        try (var conn = getConnection(); var statement = getDeleteUserPredictionsForChannelStmt(
            conn)) {
            statement.setString(1, channelId);
    ...
    @NonNull
    private PreparedStatement getDeleteUserPredictionsForChannelStmt(@NonNull Connection conn)
        throws SQLException {
    ...
    @Override
    public @NonNull Optional<String> getStreamerIdFromName(@NonNull String channelName)
        throws SQLException {
        try (var conn = getConnection();
            var statement = conn.prepareStatement("""
                SELECT ID FROM Channel WHERE Username = ?;
                """)) {
    ...
    @Override
    public @NonNull Collection<OutcomeStatistic> getOutcomeStatisticsForChannel(
        @NonNull String channelId, int minBetsPlacedByUser) throws SQLException {
        try (var conn = getConnection();
            var statement = conn.prepareStatement("""
                SELECT up.Badge,
    ...
    @Override
    public void resolvePrediction(@NonNull Event event, @NonNull String outcome,
        @NonNull String badge, double returnRatioForWin) throws SQLException {
        var ended = Optional.ofNullable(event.getEndedAt()).map(ZonedDateTime::toInstant)
            .orElseGet(TimeFactory::now);
    ...
    @Override
    public void addPrediction(@NonNull String channelId, @NonNull String eventId,
        @NonNull String type, @NonNull String description, @NonNull Instant instant)
        throws SQLException {
        try (var conn = getConnection();
            var statement = conn.prepareStatement("""
    ...
    @Override
    public void addBalance(@NonNull String channelId, int balance, @Nullable String reason,
        @NonNull Instant instant) throws SQLException {
        try (var conn = getConnection();
            var statement = conn.prepareStatement("""
